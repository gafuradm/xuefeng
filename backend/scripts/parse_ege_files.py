# backend/scripts/parse_ege_files.py
import os
import re
import json
...
def parse_task(block, subject, source):
    ...
    question = block[:800]
    answer = ""
    ans_match = re.search(r'ответ[：:]\s*(\w+)', block, re.IGNORECASE)
    if ans_match:
        answer = ans_match.group(1)
    topic = "general"
    if "функция" in block or "уравнение" in block: topic = "алгебра"
    elif "геометри" in block: topic = "геометрия"
    elif "производн" in block: topic = "математический анализ"
    difficulty = "medium"
    if len(block) < 200:
        difficulty = "easy"
    elif len(block) > 600:
        difficulty = "hard"
    return {
        "id": f"ege_{subject}_{number}_{source[:10]}",
        "source": source,
        "number": number,
        "subject": subject,
        "topic": topic,
