# backend/scripts/build_gaokao_vector.py
import sys
import os
...
def build_gaokao_vector_db():
    ...
    if not tasks_path.exists():
        print("❌ Файл gaokao_tasks.json не найден. Сначала запустите парсер.")
        return

    with open(tasks_path, 'r', encoding='utf-8') as f:
        tasks = json.load(f)

    print(f"📚 Загружено {len(tasks)} задач GAOKAO")
    ...
    # Сохраняем векторы
    vectors_dir = base_dir / "data" / "vector_stores" / "gaokao"
