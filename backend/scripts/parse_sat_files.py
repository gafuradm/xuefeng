...
SUBJECT_MAP = {
    ...
    "writing": "чтение и письмо",
    "essay": "чтение и письмо"
}
...
def split_tasks(text):
    # SAT часто использует номера 1., 2. и т.д.
    lines = text.split('\n')
    tasks = []
    current = []
    for line in lines:
...
def parse_task(block, subject, source):
    ...
    # Определение темы (примитивное)
    topic = "general"
    if "algebra" in block.lower() or "equation" in block.lower():
        topic = "алгебра"
    elif "graph" in block.lower():
        topic = "анализ данных"
    elif "probability" in block.lower():
        topic = "вероятность"
    elif "geometry" in block.lower():
        topic = "геометрия"
    difficulty = "medium"
    if len(block) < 200:
        difficulty = "easy"
        ...
        difficulty = "hard"
    return {
        "id": f"sat_{subject}_{number}_{source[:10]}",
        "source": source,
        "number": number,
        "subject": subject,
        "topic": topic,
