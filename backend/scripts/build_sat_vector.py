# backend/scripts/build_sat_vector.py
import sys
import os
...
def build_sat_vector_db():
    base_dir = Path(__file__).parent.parent
    tasks_path = base_dir / "data" / "sat" / "sat_tasks.json"
    if not tasks_path.exists():
        print("❌ Файл sat_tasks.json не найден. Сначала запустите парсер.")
        return

    with open(tasks_path, 'r', encoding='utf-8') as f:
        tasks = json.load(f)

    print(f"📚 Загружено {len(tasks)} задач SAT")
    ...
    vectors_dir = base_dir / "data" / "vector_stores" / "sat"
